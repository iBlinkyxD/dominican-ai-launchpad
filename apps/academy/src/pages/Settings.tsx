...
import { getSubscription, cancelSubscription, reactivateSubscription, StripeSubscription } from "@/api/billing";
...
useEffect(() => {
  ...
  const handleAvatarUpload = async () => {
    ...
    try {
      ...
      setAvatar(null);
      if (user) login({ ...user, profile_picture_url: avatarUrl });
                    ...
                    <div className="border border-gray-200 rounded-xl p-6">
                      ...
                      {subscription.cancel_at_period_end ? (
                        <button
                          ...
                          disabled={subActing}
                          className="flex items-center gap-2 px-5 py-2 bg-[#0B1E40] text-white text-sm font-medium rounded-lg hover:bg-[#0B1E40]/90 transition disabled:opacity-50"
                        >
